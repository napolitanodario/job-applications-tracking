...
Job alerts (LinkedIn `jobalerts-noreply`), OTP/login codes, and incomplete drafts (for example GenieAI) are excluded. Personal details (phone numbers, application copies, tokens) are not stored in the CSV.
...
| `azienda_ha_risposto` | `si` if the company/ATS sent a status email beyond LinkedIn/board "application sent" |
...
- **11** received / under review
- **17** sent, no company reply yet
- **6** rejected
